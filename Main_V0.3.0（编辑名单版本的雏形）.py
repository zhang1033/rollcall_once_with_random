...
def name_rand(name_rand_times=0):
    ...
    main.after(100,name_rand,name_rand_times+1)#防止tkintr线程阻塞，界面卡死，隔100ms再进行随机数
...
def edit_config(exist=None):
    ...
    try:
        def create():
            ...
            selected_name=tk.Label(control_frame,text="未选择",bg="#a0a0ff",fg="#0a0a0a",font=("楷体",16,"bold"))
            ...
            replace_config_button=tk.Button(control_frame,text="保存到配置文件",bg="#8383d3",fg="#fefeff",relief="flat",activebackground="#8080d0",activeforeground="#dedee3",state="disable",font=("楷体",14),wraplength=95)
            replace_config_button.place(x=30,y=160,width=100,height=50)
            replace_ram_list=tk.Button(control_frame,text="在加载后的配置中修改",bg="#8383d3",fg="#fefeff",relief="flat",activebackground="#8080d0",activeforeground="#dedee3",state="disable",font=("楷体",14),wraplength=135)
            replace_ram_list.place(x=150,y=160,height=50,width=140)
            checking=tk.StringVar()
            def selected_action(event):
                if name_list.curselection()==():
                    selected_name.config(text="未选择")
                    for i in [replace_config_button,replace_ram_list]:
                        i.config(state="disable")
                    window.after_cancel(check_name)
                else:
                    selected=True
                    selected_name_nmu=name_list.curselection()[0]
                    selected_name.config(text=f"修改已选择：{name_db_temp[selected_name_nmu]}")
                    def is_type_name_p(event):
                        def is_type_name():
                            if (selected_entry.get()=="") or (selected==False):
                                replace_config_button.config(state="disable")
                                replace_ram_list.config(state="disable")
                                print("NO")
                            else:
                                def apply_config(write_config=False,name_index=None):
                                    if selected_entry.get() in name_db_temp:
                                        ...
                                        for i in [replace_config_button,replace_ram_list]:
                                            i.config(state="disable")
                                        print(name_db_temp)
                                replace_config_button.config(state="normal",command=lambda:apply_config(write_config=True,name_index=selected_name_nmu))
                                replace_ram_list.config(state="normal",command=lambda:apply_config(write_config=False,name_index=selected_name_nmu))
                        check_name=window.after(300,is_type_name)
                    print(checking.get())
                selected_entry.bind("<Key>",is_type_name_p)
                print(name_list.curselection())
            check_name=window.after(0,None)
