...
class Win32_MessageboxW_Base:
    def __init__(self,Owner,Message_Text,Message_Title,Message_Type):
    ...
    def MessageBox_return_value(self,Value):
        ...
        #Win32_MessageBox.join(timeout=1)
#c.windll.shcore.SetProcessDpiAwareness(2) ##这个就是声明高dpi缩放替代中的“应用程序”选项，千万不要使用，在200%以上4k分辨率中会出BUG。
...
def name_rand(name_rand_times=0):
    ...
    if LOCAL_MESSAGE=="NAMELIST_UPDATED":
...
def about_me(exist=None):
    ...
    try:
        def create():
            window=tk.Toplevel()
            window.title("关于我")
            window.iconbitmap(bitmap=sys.executable)
    ...
    try:
        def create():
            ...
            def update_state(event):
                window.update()
            main.bind("<FocusIn>",update_state)
            main.bind("<FocusOut>",update_state)
            def add_namelist():
                ...
                if not primary_word==search_entry.get():
                    if (search_entry.get()=="") and (entered==1):
                        ...
                        if search_selection_conv()[1]:
                            ...
                            name_list.after(after_insert_time,lambda:name_list.selection_set(restore))
                            name_list.after(after_insert_time,lambda:name_list.see(restore))
                        else:
                            restore_namelist()
                        entered=0
                    else:
    ...
    try:
        ...
        if os.path.isfile("./namelist.json")==True:
            ...
            with open("./namelist.json","r",encoding=encoding) as namelist:
                ...
                if is_error_empty==1:
                    LOCAL_MESSAGE="NAMELIST_UPDATED"
                name_database=names
                if not len(name_database)==len(list(set(name_database))):
                    is_view_redundant_teams=Win32_MessageboxW_Base(
                        main,
                        """检测到配置文件中的元素重复，程序自动应用去重的名单，名单不会保存到配置文件中。
是否立即查看重复的元素的情况？""",
                        "随机抽取-配置文件的元素重复",
                        0x31
                        )
                    def a():
                        if is_view_redundant_teams.MessageBox_return==None:
                            main.after(100,a)
                        else:
                            if is_view_redundant_teams.MessageBox_return==1:
                                view_redundant_teams=tk.Toplevel(main,bg="#000000")
                                view_redundant_teams.minsize(400,300)
                                ...
                                buttons_below.pack(fill="x",ipady=10,side="bottom")
                                button_ok=ttk.Button(buttons_below,text="确定")
                                button_ok.pack(side="right",anchor="center",padx=10,pady=3)
                                ...
                                redundant_teams_table.pack(fill="both")
                                for i in view_redundant_teams_columns:
                                    redundant_teams_table.heading(i,text=i)
                                test_team=[["1","1"]]
                                for j in test_team:
                                    redundant_teams_table.insert("","end",values=j)
                    a()
                del names
