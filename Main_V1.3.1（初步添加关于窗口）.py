...
import main_strings as strings
import os,sys,json,time,gc
LOCAL_MESSAGE={}
view_redundant_window=[]
pre_window_errors={}
no_value=object()
self_settings_default={"is_rollcall_once":False,"apply_config_noerror":False}
        ...
        if action=="load":
            ...
            if not value is no_value:
                ...
                try:
                    ...
                    if not configs_access("apply_config_noerror"):
...
def about_me(exist=None):
    ...
    try:
        def create():
            ...
            window.minsize(int(400*dpi),int(340*dpi))
            window.resizable(False,False)
            about_me_frame=tk.Frame(window,bg="#FFFFFF")
            about_me_frame.place(x=10*dpi,y=10*dpi,height=320*dpi,width=380*dpi)
            test1=tk.Scrollbar(about_me_frame)
            test1.pack(side="right",fill="y")
            about_me=tk.Text(about_me_frame,wrap="char",relief="flat",selectbackground="#FFFFFF",yscrollcommand=test1.set)
            test=tk.Label(about_me,text=strings.About_strings.about_me(),wraplength=380*dpi,justify="left",anchor="n",font=("楷体",6))
            test.pack(side="left",anchor="n")
            about_me.yview("scroll",1,"units")
##            about_me.insert("end",strings.About_strings.about_me_leader()+"\n","leader")
##            about_me.insert("end",strings.About_strings.about_me()+"\n","about_me")
            about_me.pack()
            #about_me.bind("<Button-1>",roll)
            #about_me.tag_config("leader",foreground="#000000",font=("楷体",16,"bold"))
            #about_me.tag_config("about_me",foreground="#000000",font=("楷体",14))
            about_me.config(state="disabled")
            return(window)
        if exist==None:
