...
set_menu.add_checkbutton(label="隐藏标题栏",command=hide_title,variable=hide_title_state)
reload=tk.Menu(set_menu,tearoff=False,bg="#50E885",activebackground="#73d3A0",activeforeground="#102305")
set_menu.add_cascade(label="重新加载",menu=reload)
def reload_utf_8():
    start.config(state="normal")
    json_config(encoding="utf-8")
def reload_utf_8_bom():
    start.config(state="normal")
    json_config(encoding="utf-8-sig")
reload.add_command(label="以UTF-8编码重新加载",command=reload_utf_8)
...
def replace_xy(event):
    ...
    primary_x=event.x
    primary_y=event.y
def json_config(encoding="utf-8"):
