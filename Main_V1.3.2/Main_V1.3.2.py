...
def pre_window_error_show(text,title,types):
    global pre_window_errors
    pre_window_errors[title]=[text,types]
...
def about_me(exist=None):
    ...
    try:
        def create():
            ...
            about_me_buttons.pack(fill="x",ipady=25,side="bottom")
            ...
            about_me_button_ok.place(y=(50-about_me_button_ok.winfo_height())/2)
            about_me=[
                ...
                (tk.Label(about_me_frame,text=mainstr.About_strings.about_me_leader()[1],font=("楷体",16),fg="#0E5F61",bg="#D5F7F0"),
                 window.winfo_width()-int(about_me_canvas_scroll.cget("width")),40,"ne"),
            ...
            tar_height=about_me[-1][2]+about_me[-1][0].winfo_reqheight()+20*dpi
            ...
            for widget in about_me:
                widget[0].place(x=widget[1]*dpi,y=widget[2]*dpi,anchor=widget[3])
            ...
            def touch_screen(event):
                if str(event.widget.winfo_toplevel())==str(window) and (len(str(event.widget).split("."))>=3):
                    if str(event.widget).split(".")[2]==str(about_me_canvas).split(".")[2]:
                        origin_y=event.y_root
                        moved_length=None
                        def touch_screen_move(event):
                            nonlocal moved_length
                            if str(event.widget.winfo_toplevel())==str(window) and (len(str(event.widget).split("."))>=3):
                                if (not moved_length==None) and (str(event.widget).split(".")[2]==str(about_me_canvas).split(".")[2]):
                                    about_me_canvas.yview_scroll(int((origin_y-moved_length-event.y_root)/10),"units")
                                    moved_length=origin_y-event.y_root
                                else:
                                    moved_length=origin_y-event.y
                        about_me_canvas.bind_all("<B1-Motion>",touch_screen_move)
            about_me_canvas.bind_all("<ButtonPress-1>",touch_screen)
...
def about_program(exist=None):
    ...
    try:
        def create():
            ...
            about_program_buttons.pack(fill="x",ipady=25,side="bottom")
            ...
            about_program_button_ok.place(y=(50-about_program_button_ok.winfo_height())/2)
            wrpleng=about_program_canvas.winfo_width()-int(about_program_canvas_scroll.cget("width"))-10*dpi
            about_program=[
                (tk.Label(about_program_frame,text=mainstr.About_strings.program_new(),font=("楷体",12),fg="#0E5F61",bg="#D5F7F0",wraplength=wrpleng,justify="left"),
                 5*dpi,0,"nw"),
                (tk.Label(about_program_frame,text=mainstr.About_strings.create_late(),font=("仿宋",12),wraplength=wrpleng,justify="left",fg="#054223",bg="#D5F7F0"),
                 5*dpi,10*dpi,"nw"),
                (tk.Label(about_program_frame,text=mainstr.About_strings.about_program(),font=("仿宋",12),fg="#0E5F61",bg="#D5F7F0",wraplength=wrpleng,justify="left"),
                 5*dpi,10*dpi,"nw"),
                (tk.Label(about_program_frame,text=mainstr.About_strings.no_using_electron(),font=("仿宋",12),fg="#020E47",bg="#D5F7F0",wraplength=wrpleng,justify="left"),
                 5*dpi,0,"nw"),
                (tk.Label(about_program_frame,text=mainstr.About_strings.my_truth_opinion(),font=("仿宋",12),fg="#472722",bg="#D5F7F0",wraplength=wrpleng,justify="left"),
                 5*dpi,0,"nw"),
                (tk.Label(about_program_frame,text=mainstr.About_strings.about_program_handout(),font=("仿宋",12),fg="#0E5F61",bg="#D5F7F0",wraplength=wrpleng,justify="left"),
                 5*dpi,0,"nw"),
                (tk.Label(about_program_frame,text=mainstr.About_strings.tested_list(),font=("仿宋",12),fg="#0A334A",bg="#D5F7F0",wraplength=wrpleng,justify="left"),
                 5*dpi,20*dpi,"nw")
                      ]
            ...
            def place_text(is_place=False,skipped_index=[],change_page=False):
                nonlocal widget_total_y
                for widget in about_program:
                    if is_place:
                        if about_program.index(widget) in skipped_index:
                            widget[0].place(x=widget[1],y=widget_total_y+widget[2],anchor=widget[3])
                ...
                if not is_place:
                    ...
                    if widget_total_y>about_program_canvas.winfo_height():
            ...
            def touch_screen(event):
                if str(event.widget.winfo_toplevel())==str(window) and (len(str(event.widget).split("."))>=3):
                    if str(event.widget).split(".")[2]==str(about_program_canvas).split(".")[2]:
                        origin_y=event.y_root
                        moved_length=None
                        def touch_screen_move(event):
                            nonlocal moved_length
                            if str(event.widget.winfo_toplevel())==str(window) and (len(str(event.widget).split("."))>=3):
                                if (not moved_length==None) and (str(event.widget).split(".")[2]==str(about_program_canvas).split(".")[2]):
                                    about_program_canvas.yview_scroll(int((origin_y-moved_length-event.y_root)/10),"units")
                                    moved_length=origin_y-event.y_root
                                else:
                                    moved_length=origin_y-event.y
                        about_program_canvas.bind_all("<B1-Motion>",touch_screen_move)
            about_program_canvas.bind_all("<ButtonPress-1>",touch_screen)
            def mouse_wheel(event):
...
是否立即查看重复的元素的情况？""",
    ...
    def show_pre_window_error():
        ...
        for error in pre_window_errors:
            c.windll.user32.MessageBoxW(main.winfo_id(),pre_window_errors[error][0],error,pre_window_errors[error][1])
        pre_window_errors={}
