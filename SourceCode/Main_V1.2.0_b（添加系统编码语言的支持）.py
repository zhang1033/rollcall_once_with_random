...
def load_settings(encoding=None):
    ...
    if os.path.isfile("./config.json"):
        ...
        if os.path.isdir("./namelist.json")==True:#检测配置是否为文件夹
            pre_window_error_show("json配置为目录，请在当前目录下的\"config.json\"文件夹删除，然后再创建此文件","随机抽取-找不到json配置",0x10)
        else:
            pre_window_error_show("未找到json配置，请在当前目录下新建\"config.json\"文件","随机抽取-找不到json配置",0x10)
load_settings()
...
是否立即查看重复的元素的情况？""",
            ...
            if os.path.isdir("./namelist.json")==True:#检测配置是否为文件夹
                c.windll.user32.MessageBoxW(main.winfo_id(),"json名单为目录，请在当前目录下的\"namelist.json\"文件夹删除，然后再创建此文件","随机抽取-找不到json名单",0x10)
                if is_error_empty==0:
                    start.config(state="disable")
            else:
                c.windll.user32.MessageBoxW(main.winfo_id(),"未找到json名单，请在当前目录下新建\"namelist.json\"文件","随机抽取-找不到json名单",0x10)
                if is_error_empty==0:
                    start.config(state="disable")
    except json.decoder.JSONDecodeError as json_error:#检查json语法是否正确
