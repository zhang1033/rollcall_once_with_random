    ...
    try:
        ...
        def __init__(self):
            ...
            def edit_frame_class(
                ...
                buttonbg="#8383d3",
                buttonfg="#fefeff",
                button_activebg="#8080d0",
                button_activefg="#dedee3",
                relief="flat",
                width=150,
                height=280,
                page_bar_height=30,
                ...
                pages=len(name_database)//page_size
                if pages*page_size<len(name_database):
                    pages=pages+1
                entry_width=(width-page_bar_width*2-split_width)/2
                edit_frame=tk.Frame(window,bg=background)
                edit_frame.place(x=10,y=50,height=height,width=width)
                name_list_title=tk.Label(edit_frame,text="名称列表",bg=background,fg=activebg,font=("楷体",16,"bold"))
                name_list_title.pack(padx=5,pady=5,anchor="nw")
                name_list_page_frame=tk.Frame(edit_frame)
                name_list_page_frame.pack(side="bottom",fill="x",ipady=15)
                name_list_page_frame.config(bg=background)
                name_list_page_pre=tk.Button(name_list_page_frame,text="<",font=(30),relief=relief,bg=buttonbg,fg=buttonfg,activebackground=button_activebg,activeforeground=button_activefg)
                name_list_page_pre.place(height=page_bar_height,width=20)
                name_list_page_nex=tk.Button(name_list_page_frame,text=">",font=(30),relief=relief,bg=buttonbg,fg=buttonfg,activebackground=button_activebg,activeforeground=button_activefg)
                name_list_page_nex.place(x=width-page_bar_width,y=0,height=page_bar_height,width=20)
                name_list_pages_entry=tk.Entry(name_list_page_frame,bg=foreground,relief=relief,fg=activebg,font=("楷体",18),insertbackground="#fefeff")
                name_list_pages_entry.place(x=page_bar_width,y=0,height=page_bar_height,width=entry_width)
                name_list_page_split=tk.Label(name_list_page_frame,bg=foreground,relief=relief,fg=activebg,font=("楷体",18),text="/")
                name_list_page_split.place(x=page_bar_width+entry_width,y=0,height=page_bar_height,width=split_width)
                name_list_page_max=tk.Entry(name_list_page_frame,bg=foreground,relief=relief,fg=activebg,font=("楷体",18),insertbackground="#fefeff",readonlybackground="#8383d3")
                name_list_page_max.insert(0,str(pages))
                name_list_page_max.config(state="readonly")
                name_list_page_max.place(x=page_bar_width+entry_width+split_width,y=0,height=page_bar_height,width=entry_width)
                edit_list_frame=tk.Frame(edit_frame,bg=background,relief=relief)
                ...
                return(name_list,list_bar,page_size)
            name_list,list_bar,page_size=edit_frame_class()
            ...
            stopped=tk.IntVar()
            def add_namelist():
                for name_index in range(0,page_size//100+1,1):
                    if (name_index+1)*100>=page_size:
                        rec=name_db_temp[100*name_index:page_size:1]
                    else:
                        rec=name_db_temp[100*name_index:100*(name_index+1):1]
                    yield rec
            def test_1(i=None):
                def test_next(i=None):
                    try:
                        if i==None:
                            i=add_namelist()
                        ran=next(i)
                        name_list.insert("end",*ran)
                        window.after(10,test_next,i)
                    except StopIteration:
                        pass
                test_next()
            name_list.after(running_time_compute(start_create_time,time.perf_counter())+10,test_1)
...
set_menu.add_command(label="编辑配置文件",command=lambda:edit_config())
...
main.after(running_time_compute(main_window_time,main_window_finish_time),json_config)
#防止主窗口消失
main.mainloop()
