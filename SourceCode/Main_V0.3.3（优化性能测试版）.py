...
def edit_config(exist=None):
    ...
    try:
        def create():
            ...
            def add_namelist(add_len,times=0):
                temp_name_list=[]
                temp_name_list.clear()
                if add_len*(times+1)<=len(name_db_temp):
                    for name_index in range(add_len*times,add_len*(times+1),1):
                        temp_name_list.append(name_db_temp[name_index])
                    name_list.after(10,lambda:add_namelist(add_len,times+1))
                else:
                    for name_index in range(add_len*times,len(name_db_temp),1):
                        temp_name_list.append(name_db_temp[name_index])
                name_list.insert("end",*temp_name_list)
            test1=thd.Thread(target=test11)
            test=thd.Thread(target=add_namelist,args=(150,0))
            test1.start()
            name_list.after(running_time_compute(start_create_time,time.perf_counter())+10,lambda:test.start())
