...
def edit_config(exist=None):
    ...
    try:
        def create():
            ...
            def add_namelist():
                #if len(name_db_temp)<=1500:
                print((len(name_db_temp)-len(name_db_temp)%1000)/1000)
                for name_index in range(0,int((len(name_db_temp)-len(name_db_temp)%1000)/1000)+2,1):
                    def a():
                        nonlocal name_index,name_db_temp
                        name_list.insert("end",*name_db_temp[1000*(name_index-1):1000*name_index:1])
                        name_list.update()
                    name_list.after(8,a)
