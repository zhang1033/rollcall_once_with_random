...
def edit_config(exist=None):
    ...
    try:
        def create():
            ...
            add_del_opinion=tk.Menubutton(control_frame,state="disable",bg="#8383d3",fg="#fefeff",relief="flat",activebackground="#8383d3",activeforeground="#fefeff",font=("楷体",14),wraplength=210)
            add_del_opinion.place(x=15,y=200,width=215,height=30)
            add_del_opinion_menu=tk.Menu(add_del_opinion,tearoff=False,bg="#8383d3",fg="#fefeff",activebackground="#8383d3",activeforeground="#fefeff")
            add_del_opinion.config(menu=add_del_opinion_menu)
            edit_direction=0
            def entry_type(selected,name=""):
                def is_selected():
                    if not selected==0:
            ...
            def add_del_opinions(selected=0,behind=1,pre_set=1):
                ...
                def set_a_opinion(behind_i):
                    ...
                    edit_direction=behind_i
                set_a_opinion(behind)
                if pre_set==0:
            ...
            def delete_t(name_index):
                ...
                if name_list.get(name_list.size()-1)==name_list.get(name_index):
                    ...
                    if state_add_del_opinion():
                        selected_name.config(text=f"修改已选择：{name_db_temp[-1]}")
                    else:
                        selected_name.config(text="列表为空，请添加一项。")
                else:
                    ...
                    del name_db_temp[name_index]
                    selected_name.config(text=f"修改已选择：{name_db_temp[name_index]}")
             ##推到重做吧（已完善一些输入机制）
            def apply_config(name_index=None):
                if selected_entry.get() in name_db_temp:
                    ...
                    name_list.selection_set(name_index)
                    selected_name.config(text=f"修改已选择：{name_db_temp[name_list.curselection()[0]]}")
                    replace_list.config(state="disable")
                    #print(name_db_temp)
            def selected_action(event):
                if name_list.curselection()==():
                    if name_db_temp==[]:
                        ...
                        replace_list.config(state="disable")
                        delete_list_t.config(state="disable")
                        add_del_opinions(selected=0)
                    def entry_type_1():
                        if not selected_entry.get()=="":
                            add_list_t.config(state="normal")
                        else:
                            add_list_t.config(state="disable")
                        window.after(100,entry_type_1)
                    window.after(0,entry_type_1)
                else:
                    selected_name.config(text=f"修改已选择：{name_db_temp[name_list.curselection()[0]]}")
                    delete_list_t.config(state="normal")
                    add_del_opinions(selected=1)
                    delete_list_t.config(command=lambda:delete_t(name_list.curselection()[0]))
                    def entry_type():
                        if not name_list.curselection()==():
                            if not selected_entry.get()=="":
                                add_list_t.config(state="normal")
                                replace_list.config(state="normal",command=lambda:apply_config(name_index=name_list.curselection()[0]))
                            else:
                                add_list_t.config(state="disable")
                                replace_list.config(state="disable")
                        window.after(100,entry_type)
                    window.after(0,entry_type)
